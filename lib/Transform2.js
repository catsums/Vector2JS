...
class Transform2 {
    ...
    _childs = [];
    static get ORIGIN() {
        return new Transform2(Vector2_1.Vector2.ZERO, 0, Vector2_1.Vector2.ONE, Vector2_1.Vector2.ZERO, Vector2_1.Vector2.ZERO);
    }
    static EQUALS(t1, t2, p = 0) {
        ...
        if (!t2 || !t2)
            return false;
        if (Vector2_1.Vector2.EQUALS(t1.position, t2.position, p) &&
            Vector2_1.Vector2.EQUALS(t1.scale, t2.scale, p) &&
            Vector2_1.Vector2.EQUALS(t1.skew, t2.skew, p) &&
            Vector2_1.Vector2.EQUALS(t1.anchor, t2.anchor, p))
            return true;
        return false;
    }
    ...
    static INVERSE(t) {
        let p = Vector2_1.Vector2.NEG(t.position);
        let s = Vector2_1.Vector2.INVERSE(t.scale);
        let r = -(t.rotation);
        let k = Vector2_1.Vector2.NEG(t.skew);
        let a = t.anchor;
    ...
    set position(x) {
        if (x instanceof Vector2_1.Vector2)
            this._position = new Vector2_1.Vector2(x);
    }
    ...
    set scale(x) {
        if (x instanceof Vector2_1.Vector2)
            this._scale = new Vector2_1.Vector2(x);
    }
    get skew() { return this._skew; }
    set skew(x) {
        if (x instanceof Vector2_1.Vector2)
            this._skew = new Vector2_1.Vector2(x);
    }
    get anchor() { return this._anchor; }
    set anchor(x) {
        if (x instanceof Vector2_1.Vector2)
            this._anchor = new Vector2_1.Vector2(x);
    }
    ...
    constructor(...args) {
        if (args[0] instanceof Transform2) {
            ...
            this.scale = new Vector2_1.Vector2(p.scale);
            this.skew = new Vector2_1.Vector2(p.skew);
            this.anchor = new Vector2_1.Vector2(p.anchor);
        }
        else {
            let [p, r, s, k, a] = args;
            this.position = new Vector2_1.Vector2(p);
            this.rotation = (r);
            this.scale = new Vector2_1.Vector2(s);
            this.skew = new Vector2_1.Vector2(k);
            this.anchor = new Vector2_1.Vector2(a);
        }
    }
    inverted() {
    ...
    getGlobalTransform() {
        ...
        let p = Vector2_1.Vector2.ADD(pt.position, this.position);
        let r = pt.rotation + this.rotation;
        let s = Vector2_1.Vector2.MULTIPLY(pt.scale, this.scale);
        let k = new Vector2_1.Vector2(Math.tan(Math.atan(this.skew.x) + Math.atan(pt.skew.x)), Math.tan(Math.atan(this.skew.y) + Math.atan(pt.skew.y)));
    ...
    applyGlobalTransform(pt, anchor = this.anchor, order = ['S', 'K', 'R', 'T']) {
        ...
        let newPt = new Vector2_1.Vector2(pt);
    ...
    applyTranslate(pt, anchor = this.anchor) {
        let newPt = new Vector2_1.Vector2(pt);
        newPt = Vector2_1.Vector2.SUBTRACT(newPt, anchor);
        newPt = Vector2_1.Vector2.ADD(newPt, this.position);
        newPt = Vector2_1.Vector2.ADD(newPt, anchor);
        return newPt;
    }
    applyRotate(pt, anchor = this.anchor) {
        let newPt = new Vector2_1.Vector2(pt);
        newPt = Vector2_1.Vector2.SUBTRACT(newPt, anchor);
        newPt = newPt.rotated(Vector2_1.Vector2.ZERO, this.rotation);
        newPt = Vector2_1.Vector2.ADD(newPt, anchor);
        return newPt;
    }
    applySkew(pt, anchor = this.anchor) {
        let newPt = new Vector2_1.Vector2(pt);
        newPt = Vector2_1.Vector2.SUBTRACT(newPt, anchor);
        newPt = newPt.skewed(Vector2_1.Vector2.ZERO, this.skew);
        newPt = Vector2_1.Vector2.ADD(newPt, anchor);
        return newPt;
    }
    applyScale(pt, anchor = this.anchor) {
        let newPt = new Vector2_1.Vector2(pt);
        newPt = Vector2_1.Vector2.SUBTRACT(newPt, anchor);
        newPt = Vector2_1.Vector2.MULTIPLY(newPt, this.scale);
        newPt = Vector2_1.Vector2.ADD(newPt, anchor);
        return newPt;
    }
    ...
    applyTransform(pt, anchor = this.anchor, order = ['S', 'K', 'R', 'T']) {
        ...
        for (let trans of order) {
            ...
            switch (trans?.toUpperCase()) {
                ...
                case 'TRANSLATE':
                case 'POSITION':
                    newPt = this.applyTranslate(newPt, Vector2_1.Vector2.ZERO);
                    break;
                case 'R':
                case 'ROTATE':
                case 'ROTATION':
                    newPt = this.applyRotate(newPt, Vector2_1.Vector2.ZERO);
                    break;
                case 'K':
                case 'SKEW':
                    newPt = this.applySkew(newPt, Vector2_1.Vector2.ZERO);
                    break;
                case 'S':
                case 'SCALE':
                case 'SIZE':
                    newPt = this.applyScale(newPt, Vector2_1.Vector2.ZERO);
                    break;
                default:
                    newPt = newPt;
                    break;
