...
export class Transform2 {
    ...
    static EQUALS(t1, t2, p = 0) {
        ...
        if (!t2 || !t2)
            return false;
        if (Vector2.EQUALS(t1.position, t2.position, p) &&
    ...
    getGlobalTransform() {
        ...
        let k = new Vector2(Math.tan(Math.atan(this.skew.x) + Math.atan(pt.skew.x)), Math.tan(Math.atan(this.skew.y) + Math.atan(pt.skew.y)));
