...
class Transform2 {
  ...
  _childs = [];
  static get ORIGIN() {
    return new Transform2(_Vector.Vector2.ZERO, 0, _Vector.Vector2.ONE, _Vector.Vector2.ZERO, _Vector.Vector2.ZERO);
  }
  ...
  static SIMILAR(t1, t2, p = 0) {
    if (t1 === t2) return true;
    if (!t2 || !t2) return false;
    if (_Vector.Vector2.EQUALS(t1.position, t2.position, p) && _Vector.Vector2.EQUALS(t1.scale, t2.scale, p) && _Vector.Vector2.EQUALS(t1.skew, t2.skew, p) && MY.roundTo(r, p) === MY.roundTo(r, p)) return true;
    return false;
  }
  static INVERSE(t) {
