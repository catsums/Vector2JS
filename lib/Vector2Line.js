...
class Vector2Line {
    ...
    get gradient() {
        return (0, MY_1.safeDivide)(-(this.a), this.b);
    }
    get m() {
        return this.gradient;
    }
    get xIntercept() {
        return (0, MY_1.safeDivide)(-(this.c), this.a);
    }
    get f() {
        return this.xIntercept;
    }
    get yIntercept() {
        return (0, MY_1.safeDivide)(-(this.c), this.b);
    }
    get e() {
    ...
    constructor(...args) {
        if (args[0] instanceof Vector2Line) {
            ...
            let a, b, c;
            let v1 = new Vector2_1.Vector2((x.x || x[0] || 0), (x.y || x[1] || 0));
            let v2 = new Vector2_1.Vector2((y.x || y[0] || 0), (y.y || y[1] || 0));
            let m = Vector2_1.Vector2.SUBTRACT(v2, v1).gradient();
            //y=mx+e
            //e = y - mx
            let e, f;
            if ((0, MY_1.isInfinity)(m)) {
                //vertical line
                c = -(v1.x || v2.x);
                b = 0;
                a = 1;
                ...
                e = (v1.y) - (m * (v1.x));
                f = (0, MY_1.safeDivide)(-e, m);
                c = -(f * m);
                b = (0, MY_1.safeDivide)(-c, e) || 1;
                a = -(b * m);
                //
            }
            ...
            if ('a' in x && 'b' in x && 'c' in x) {
                ...
                if ('f' in obj || 'xIntercept' in obj) {
                    ...
                    c = 1;
                    f = (0, MY_1.safeDivide)(-c, m);
                }
                b = (0, MY_1.safeDivide)(-c, e);
                a = -(b * m);
                this.a = a;
                this.b = b;
                this.c = c;
                ...
                let m = new Vector2_1.Vector2(0 - e, f - 0).gradient();
                c = -(f * m);
                b = (0, MY_1.safeDivide)(-c, e);
                a = -(b * m);
                this.a = a;
    ...
    getX(y) {
        ...
        if (this.b != 0) {
            ...
            //y = mx+e
            //x = (y - e)/m
            x = (0, MY_1.safeDivide)((y - e), m);
        }
        else {
            let c = this.c;
            let a = this.a;
            x = (0, MY_1.safeDivide)(-c, a);
        }
        return x;
    }
    getY(x) {
    ...
    hasPoint(point) {
        let v = new Vector2_1.Vector2(point);
        //ax+by+c=0
        let res;
        if ((0, MY_1.isInfinity)(this.a) && (0, MY_1.isInfinity)(this.b)) {
            if (this.a == this.b) {
                // INF + INF
                res = Infinity + this.c;
            }
    ...
    angle() {
        ...
        if (xInt == Infinity)
            return 0; //or Math.PI
        if (xInt == -Infinity)
            return (Math.PI); //or Math.PI
        if (yInt == Infinity)
            return (Math.PI / 2);
        if (yInt == -Infinity)
            return -(Math.PI / 2);
        let vx = new Vector2_1.Vector2(xInt, 0);
        let vy = new Vector2_1.Vector2(0, yInt);
    ...
    intersect(other) {
        ...
        let a1c2 = a1 * c2;
        let BC, AB, AC;
        if ((0, MY_1.isInfinity)(b1c2) && (0, MY_1.isInfinity)(b2c1) && b1c2 == b2c1) {
            BC = b1c2;
        }
        else {
            BC = (b1c2 - b2c1);
        }
        if ((0, MY_1.isInfinity)(a1b2) && (0, MY_1.isInfinity)(a2b1) && a1b2 == a2b1) {
            AB = a1b2;
        }
        else {
            AB = (a1b2 - a2b1);
        }
        if ((0, MY_1.isInfinity)(a2c1) && (0, MY_1.isInfinity)(a1c2) && a2c1 == a1c2) {
            AC = a2c1;
        }
        else {
            AC = (a2c1 - a1c2);
        }
        let x = (0, MY_1.safeDivide)((BC), (AB));
        let y = (0, MY_1.safeDivide)((AC), (AB));
        return new Vector2_1.Vector2(x, y);
    }
    perpendicular(point) {
        point = new Vector2_1.Vector2(point);
        if (this.a == 0) {
            ...
            let m = (0, MY_1.safeDivide)(-1, this.gradient);
            let e = (point.y) + ((1 / m) * point.x);
            let f = (0, MY_1.safeDivide)(-e, m);
    ...
    normal() {
        return new Vector2_1.Vector2(this.a, this.b);
    }
    mirror(point) {
        point = new Vector2_1.Vector2(point);
        if (this.hasPoint(point)) {
            return new Vector2_1.Vector2(point);
        }
        let _normal = this.normal();
        let unitNormal = _normal.normalized();
        let unitC = (0, MY_1.safeDivide)(this.c, _normal.length());
        ...
        return new Vector2_1.Vector2(mx, my);
    }
    asObject() {
