var Vector2JS = (() => {
  ...
  class Rect2 {
    ...
    static EQUALS(r1, r2, p = 0) {
      ...
      let a = new Rect2(r1);
      let b = new Rect2(r2);
      if (import_Vector2.Vector2.EQUALS(a.position, b.position, p) && import_Vector2.Vector2.EQUALS(a.size, b.size, p))
        return true;
      return false;
    }
    ...
    static getFromPoints(points) {
      ...
      for (let pt of points) {
        pt = new import_Vector2.Vector2(pt);
    ...
    get start() {
      return new import_Vector2.Vector2(this.left, this.top);
    }
    get end() {
      return new import_Vector2.Vector2(this.right, this.bottom);
    }
    get center() {
      return import_Vector2.Vector2.MIDPOINT([this.start, this.end]);
    }
    get extents() {
      return import_Vector2.Vector2.SUBTRACT(this.center, this.start);
    }
    get topLeft() {
      return new import_Vector2.Vector2(this.left, this.top);
    }
    get topRight() {
      return new import_Vector2.Vector2(this.right, this.top);
    }
    get bottomLeft() {
      return new import_Vector2.Vector2(this.left, this.bottom);
    }
    get bottomRight() {
      return new import_Vector2.Vector2(this.right, this.bottom);
    }
    ...
    constructor(...args) {
      ...
      if (p instanceof Object) {
        if ("position" in p && "size" in p) {
          ...
          this.size = new import_Vector2.Vector2(p.size);
        } else if ("x" in p && "y" in p && "w" in p && "h" in p) {
          this.position = new import_Vector2.Vector2(p.x, p.y);
          this.size = new import_Vector2.Vector2(p.w, p.h);
        } else if ("x" in p && "y" in p && "x" in s && "y" in s) {
          this.position = new import_Vector2.Vector2(p);
          this.size = new import_Vector2.Vector2(s);
        } else if ("top" in p && "left" in p && "right" in p && "bottom" in p) {
          this.position = new import_Vector2.Vector2(p.left, p.top);
          this.size = new import_Vector2.Vector2(p.right - p.left, p.bottom - p.top);
        } else if ("t" in p && "l" in p && "r" in p && "b" in p) {
          this.position = new import_Vector2.Vector2(p.l, p.t);
          this.size = new import_Vector2.Vector2(p.r - p.l, p.b - p.t);
        }
      } else if (typeof p === "number" && typeof s === "number" && typeof w === "number" && typeof h === "number") {
        this.position = new import_Vector2.Vector2(p, s);
        this.size = new import_Vector2.Vector2(w, h);
      } else {
        this.position = new import_Vector2.Vector2();
        this.size = new import_Vector2.Vector2();
      }
    }
    equals(other, p = 0) {
    ...
    containsPoint(v) {
      v = new import_Vector2.Vector2(v);
    ...
    getIntersectWith(rect, threshold = 0) {
      ...
      return new Rect2(
        new import_Vector2.Vector2(xMin, yMin),
        new import_Vector2.Vector2(xMax - xMin, yMax - yMin)
      );
    }
  ...
  return __toCommonJS(Rect2_exports);
})();
