import { mod, safeDivide, roundTo } from './MY';
export class Vector2 {
    ...
    static EQUALS(v1, v2, precision = 0) {
        ...
        if (v1?.x === v2?.x && v1?.y === v2?.y)
            return true;
        if (roundTo(v1.x, p) === roundTo(v2.x, p) &&
    ...
    ratioed() {
        ...
        return new Vector2(safeDivide(this.x, sum), safeDivide(this.y, sum));
    }
    isNormalised() {
    ...
    normalized() {
        ...
        return new Vector2(safeDivide(this.x, llen), safeDivide(this.y, llen));
    }
    normalised() {
